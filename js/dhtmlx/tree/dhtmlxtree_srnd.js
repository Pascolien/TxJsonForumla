...
dhtmlXTreeObject.prototype.prepareSR=function(it,mode){ 
	try {
		...
		try{it.htmlNode.childNodes[0].appendChild(tr);}catch(e){}
		it._sready=true;
...
this.deleteChildItems=function(id){  
	if (this.rootId==id){
		...
		this.htmlNode=new dhtmlXTreeItemObject(this.rootId,"",0,this);
   		try{this.htmlNode.htmlNode.childNodes[0].childNodes[0].style.display="none";}catch(e){}
   		try{this.htmlNode.htmlNode.childNodes[0].childNodes[0].childNodes[0].className="hiddenRow";}catch(e){}
   		
   		this.allTree.insertBefore(this.htmlNode.htmlNode,this.selectionBar);
		if(_isFF){
			try{this.allTree.childNodes[0].width="100%";}catch(e){}
			try{this.allTree.childNodes[0].style.overflow="hidden";}catch(e){}
		}
	}
}
