...
function Ouvrir_TreeviewPE(id_tree, type, id, id_branche) {
	...
	if(e1.css("display") == 'none'){
		e1.css("display",'');
		if(img){
			img.attr("src",'../../'+ Path_Graphics_Charter +'/img/btn_tree/minus.png');
		}
	} else {
		e1.css("display",'none');
		if(img){
			img.attr("src",'../../'+ Path_Graphics_Charter +'/img/btn_tree/plusik.png');			
		}
	}
	if ((e1.html() == '') && (type == 'pe')){
		e1.html('<div><img src="../../'+ Path_Graphics_Charter +'/img/gif/ajax-loader.gif" width="20px" /></div>');
		if (obj.attr("id_c") != "0"){
		...
		new J.ajax({
			...
			dataType:'html',
			data:{  
				sFunction_Name :"Get_HTML_TreeviewPE_LoadLinkAPI",
				id_tree: id_tree, 
				id_pe: id, 
...
function Select_Branche(id_tree, type, AID_Branche) {
	...
	if (test) {
		J('#smc_zone_droite').html('<div style="width: 100%; text-align:center;"><img src="../../'+ Path_Graphics_Charter +'/img/gif/ajax-loader.gif" /></div>');
		...
		new J.ajax({
			...
			dataType:'html',
			data:{
				sFunction_Name : "Get_HTML_SMC_LoadCriterion", 
				id: id, 
				type: type,
				id_attribute:obj.attr("id_met")
			},
			success: function (data) {
				J('#smc_zone_droite').html(data);
			}
		});
