...
function Table_OnCheck(AID_Table){
	...
	Table.attachEvent("onCheckbox", function(AID_Row,AID_Cell,AChecked){
		new J.ajax({
			url:'ajax/actions_grid.asp',
			async:false,
			cache:false,
			dataType:'script',
			data:{
				sFunction_Name:"Table_OnCheck",
				ID_Table:AID_Table,
...
function Table_Check_All(AID_Table,AID_Table_Linear,AChecked){
	...
	new J.ajax({
		...
		dataType:'script',
		data:{
			sFunction_Name:"Table_Check_All",
			ID_Table:ID_Table,
...
function Switch_View_Table(AID_Table,AID_Table_Linear){
	...
	if(J("#table_tree_view").css("display") == "block"){
		...
		obj_linear_grid.loadXML('ajax/load_xml_wto_table_linear.asp?id_wto='+id_treegrid+'&id_linear_wto='+ id_lineargrid);
	} else {
		//table view linear
...
function Get_ID_Object_Grid(AID_Grid,AsID_Object){
	...
	new J.ajax({
		...
		dataType:'script',
		data:{
			sFunction_Name:"Get_Id_Object_Grid",
			ID_Grid:AID_Grid,
