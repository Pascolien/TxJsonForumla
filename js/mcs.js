			...
			if (J(elts[i]).attr('class') == 'nom_'+id_tree) {
				if (J(elts[i]).attr("id_c") != '0') {
					...
					new J.ajax({
						...
						cache: false,
						data : { 
							sFunction_Name :"Write_Link_Criterion",
							id_cdc: ID_Cdc, 
...
function Delete_Criterion() {	
	...
	id_treePE = page_id_branche_select.substr(page_id_branche_select.length - page_id_branche_select.indexOf('_',3)+1);
	...
	if (verif_form2(document.form1, false)) {
		...
		if (Type_Critere == 3) { 
			new J.ajax({
				...
				dataType:'html',
				data:{
					sFunction_Name :"Write_Text_Criterion",
					id_cdc: ID_Cdc,
					id_critere_pere: id_critere_pere,
			...
			new J.ajax({
				...
				dataType:'html',
				data:{
					sFunction_Name :"Write_Boolean_Criterion",
					id_cdc: ID_Cdc,
					id_critere_pere: id_critere_pere,
					id_critere: ID_Critere,
					id_pe: obj.attr("id_met"),
			...
			if ((document.form1.inp_type_critere_num.value == 4)|| (document.form1.inp_type_critere_num.value == 5)||(document.form1.inp_type_critere_num.value == 6)) { // cas des intervalles
			    if ((borne_sup == "") || (borne_inf == "") || (parseFloat(borne_sup) < parseFloat(borne_inf))) { // on vérifie que la borne sup soit supérieur à la borne inf
				    parent.Popup_Alert(RS_JS_Erreur_borneMinMax);
					return false;
				}
			}
			// "Egal à"
			...
			new J.ajax({
				...
				dataType:'html',
				data:{
					sFunction_Name :"Write_Numerical_Criterion",
				    id_cdc: ID_Cdc,
					id_critere_pere: id_critere_pere,
					id_critere: ID_Critere,
					id_pe: obj.attr("id_met"),
			...
			if (document.form1.serie1.value == document.form1.serie2.value) {
			    parent.Popup_Alert(RS_JS_Erreur_Series);
				return false;															
			} else {
				var borne_inf = 0;
				var borne_sup = 0;
			...
			new J.ajax({
				...
				dataType:'html',
				data:{
				sFunction_Name :"Write_Graph_Criterion",
					id_pe: obj.attr("id_met"), type_critere_numerique: document.form1.inp_type_critere_num.value,
					critere_optimisation: 0, valeur_optimisation_cible: 0, id_unite_valeur_cible: 0,
					critere_inf: critere_inf, borne_inf: borne_inf, tolerance_inf: tolerance_inf, id_unite_inf: document.form1.unite.value,
					critere_sup: critere_sup, borne_sup: borne_sup, tolerance_sup: tolerance_sup, id_unite_sup: document.form1.unite.value,
					abscisse_courbe: abscissa_curve, id_colx: document.form1.serie2.value, id_coly: document.form1.serie1.value, courbe_existe: true,
					selection: Type_SMC, empty_data: Empty_Data, save_database: save_database 
				},
				success:function(data){
					obj.attr("id_c",data);
					obj.css("fontWeight","bold");
...
function DisplayMCSGraphique(aIdAttribute) {
    new J.ajax({
        ...
        dataType:'html',
        data: {
            sFunction_Name: "DisplayMCSGraphique",
            idAttribute: aIdAttribute
        },
        success: function (data) {
            J("#representation_valeur").html(data);
        }
    })
}