...
function Execute_CG_JQUERYTOCORRECT(id_cg, type) {
		...
		if (rQuestion.attr("name") && (rQuestion.attr("name").slice(0, 6) == 'inpCG_') && ((rQuestion.val()) != '') && ((rQuestion.val()) != null)) {
			...
			if (rQuestion.attr("qtype") in {'1':'', '2':'', '3':'', '5':''}) { // qtGroup, qtNumerical, qtText
				...
				if (((SL_CG_Question.eq(i+1).val()) != '') && (parseFloat((SL_CG_Question.eq(i+1)).val()) >= parseFloat((SL_CG_Question.eq(i+1)).val()))) { 
					...
					parent.Popup_Alert(text_param(RS_JSMinMax_Incorrect, rQuestion.attr("codage_nom")));
					return;
				}   			
				i = i + 1;
	...
	if (type == 'run') {
        ...
        new J.ajax({
			url:'../asp/ajax/actions_Choice_Guide.asp',
			async:false,
	        cache:false,
			method:'post',
			dataType:'script',
			data:{ 
				sFunction_Name :"Create_MCS_From_CG",
				id_cg : id_cg, 
				values: SL_Answer 
			},
		...
		if (Check_IE()) { 
			new J.ajax({
				url:'../../code/asp/ajax/actions_Choice_Guide.asp',
				dataType:'script',
				async:false,
				cache:false,
				data:{
					sFunction_Name :"Extract_CG_To_Word",
					id_cg:id_cg,
					values:SL_Answer
				}
			});
		...
		parent.window.parent.frames['frame_blanc'].location.replace('../../code/asp/ajax/export_cg_to_xml.asp?id_cg='+id_cg+'&values='+SL_Answer,'ExtractCG','height=50,width=50,toolbar=no,menubar=no');  
	}
}
...
function Execute_CG(id_cg, type) {
		...
		if ((rQuestion.name.slice(0, 6) == 'inpCG_') && ($F(rQuestion) != '') && ($F(rQuestion) != null)) {
			...
			if (rQuestion.getAttribute("qtype") in {'1':'', '2':'', '3':'', '5':''}) { // qtGroup, qtNumerical, qtText
				...
				if (($F(SL_CG_Question[i+1]) != '') && (parseFloat($F(SL_CG_Question[i+1])) >= parseFloat($F(SL_CG_Question[i])))) { 
					...
					parent.Popup_Alert(text_param(RS_JSMinMax_Incorrect, rQuestion.getAttribute("codage_nom")));
					return;
				}   			
				i = i + 1;
	...
	if (type == 'run') {
		...
		if (id_rl != 0) {
		    parent.dhxWins.window("dhxPopup_CG").hide();
		    parent.Display_Popup_MCS(0, id_rl, "resultats", true);
		    parent.dhxWins.window("dhxPopup_CG").show();
		} else {
	 	    parent.Popup_Alert("Error in generation of the requirement list.");
		}
	} else if (type == 'extract') {	
		if (Check_IE()) { 
			new J.ajax({
			    url: '../../code/asp/ajax/extract_cg_to_word.asp',
			    cache: false,
				dataType:'script',
				data:{
					id_cg:id_cg,
		    ...
		    parent.Popup_Alert(Get_RS("RS_JS_IE"));
		}
	} else if (type == 'export') {
		parent.window.parent.frames['frame_blanc'].location.replace('../../code/asp/ajax/export_cg_to_xml.asp?id_cg='+id_cg+'&values='+SL_Answer,'ExtractCG','height=50,width=50,toolbar=no,menubar=no');  
	}
