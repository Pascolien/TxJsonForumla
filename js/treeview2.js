...
function Ouvrir_Treeview(id_tree, id_te, id_pe, id_e, type, recurs, dossier_selectionnable) {
	...
	if(e1.css("display") == 'none') {
  		e1.css("display",'');
  		if(img)img.attr("src",'../../resources/theme/img/btn_tree/minus.png');
  	}else{
  		e1.css("display",'none');
  		if(img)img.attr("src",'../../resources/theme/img/btn_tree/plusik.png');
  	}  	
  	if(e1.html() == ''){
...
function Charger_Enfant(id_tree, id_te, id_pe, id_e, type, recurs, dossier_selectionnable) {
	...
	new J.ajax({
		...
		dataType:'html',
		data:{
			sFunction_Name :"Get_HTML_Treeview_LoadChild",
			id_pe: id_pe, 
...
function Load_Visible_Group_Ini(id_tree) {
    var erase_data;
  	//alert("Load__Visible_Group_Ini. id_tree: " + id_tree);
    if ((eval('window.Model_Tree_' + id_tree)) && (eval('Model_Tree_' + id_tree) > 0) && (Jn('id_ma_on_checked_' + id_tree).eq(0) != undefined) && (Jn('id_ma_on_checked_' + id_tree).eq(0).val() != '0')) {
		list_id_group = '';

        new J.ajax({   
			url:'../asp/ajax/load_visible_group.asp',
			async:false,
			cache:false,
			dataType:'script',
			data:{ id_model: eval('Model_Tree_'+id_tree), aaction: 'erase_data', list_id_select : Jn('id_e_coche_abs_'+id_tree).eq(0).val() },
			success: function(data){
			    erase_data = data;
			}
		});
		if (erase_data == "True") {
			Modalbox.show('<div class="warning">'+ RS_JS_Erase_Data +'<br /><br /><a href="." onclick="Load_Group_Visible('+id_tree+', true); Modalbox.hide(); return false;">'+ RS_JS_Yes +'</a>&nbsp;&nbsp;&nbsp;<a href="." onclick="Load_Group_Visible('+id_tree+', false); Modalbox.hide(); return false;">'+ RS_JS_No +'</a></div>', {title: '', width: 300, overlayClose: false, autoFocusing: true, hideCloseBtn: true});
		} else {
			Load_Group_Visible(id_tree, false);
		}
    } 
}

function Load_Group_Visible(id_tree, erase_data) {			
	 ...
	 new J.ajax({   
			...
			cache:false,
			dataType:'script',
			data:{ id_model: eval('Model_Tree_'+id_tree), erase_data: erase_data, aaction: 'list_id_group', list_id_select : Jn('id_e_coche_abs_'+id_tree).eq(0).val() },
			success: function(data){
				list_id_group = data;
			}
		}); 
	...
	if (erase_data == true) {
		navigation(0, 0, "write_form", false);        
    } else {
		Display_Group(Jn('id_group_parent_'+ id_tree).eq(0).val(), list_id_group);
    }  
}
...
function ClickOnRadio(id_tree, id_pe, obj) {
	...
	Load_Visible_Group_Ini(id_tree);
}
...
function Display_Read_Tree(id_tree, id_pe, searchValue,AMultiplicity) {
  	try{
		...
		new J.ajax({
		    url:'../asp/ajax/load_selected_object.asp',
		    async: false,
		    cache: false,
            type:"post",
		    dataType:'html',
...
function Hauteur_Treeview(id_tree) {
	if (J('#fleche_jaune_'+id_tree)) {
		...
		if (str_image.substr(str_image.length-13) == "bas_jaune.png") {
		  ...
		  J('#fleche_jaune_'+id_tree).attr("src",'../../resources/theme/img/btn_form/16x16_yellow_arrow_up.png');
		} else {
		  J('#fleche_jaune_'+id_tree).attr("src",'../../resources/theme/img/btn_form/16x16_yellow_arrow_down.png');
	...
	if (J('#tree1_'+id_tree+'_0').css("display") == "block"){
		...
		// Appel à une fonction de l'API qui renvoi toutes les E pour un TE et un ID_E Pere (ici 0)
		 new J.ajax({   
			...
			dataType:'script',
			data:{ 
				sFunction_Name :"Get_SL_ID_E_apd_ID_TE",
				id_te: AID_OT, 
				id_e_pere: 0, 
