...
function verif_form2(form, bool_submit) {
	try {
			...
			J(form).find(':input').each(function(){
				...
				if (objet.attr("codage") == "html_text") {
					...
					if (reg_nombre.exec(objet.val()) == null) {
						Popup_Alert(text_param(RS_JS_Entier_Incorrect,nom_caract));
						objet.val('');
					}
				} 
					...
					if ((reg_mail.exec(objet.val()) == null) && (objet.val() != '')) {
						Popup_Alert(text_param(RS_JS_Mail_Incorrect,nom_caract));
						objet.val('');
					}
				}
					...
					if (reg_nb_flottant.exec(objet.val()) == null) {
						Popup_Alert(text_param(RS_JS_Flottant_Incorrect,nom_caract));
						objet.val('');
					}
					...
					if (val1.val() != "") {
						...
						if ((val2 != null) && (val1.attr("name") == val2.attr("name"))) {
                            ...
                            if (val2.val() == '') {
                                test = false;
                                Popup_Alert(text_param(RS_JS_Max_Incorrect,nom_caract));
                                //return false;				  
                            }
                            val3 = J(this);
                            if ((val3 != null) && (val3.attr("name") == val2.attr("name"))) {
                                ...
                                if (val3.val() == '') {
                                    test = false;
                                    Popup_Alert(text_param(RS_JS_Moyenne_Incorrect,nom_caract));				  
                                }
						...
						if (reg_nb_flottant.exec(val1.val()) == null) {
						    test = false;
							Popup_Alert(text_param(RS_JS_Min_Incorrect,nom_caract));
						}
						...
						if ((test == true) && ( ((val1.val() == "") && (val2.val() != "")) || ((val1.val() != "") && (val2.val() == "")) )) {
						    test = false;
							Popup_Alert(text_param(RS_JS_MinMax_Incorrect, nom_caract));
						}
		
						if ((test == true) && (val1.val() != "") && (val2.val() != "")) {
							if (parseFloat2(val1.val()) > parseFloat2(val2.val())) {
							    test = false;
						  	    Popup_Alert(text_param(RS_JS_Min_Superieur_Max, nom_caract));
							}
						}
					} else {
					...
					if (reg_nb_monetaire.exec(objet.val()) == null) {
		 				Popup_Alert(text_param(RS_JS_Monetaire_Incorrect,nom_caract)) ;
						objet.val('');
					}
					...
					if (objet.val().length != temp.length) {
						...
						Popup_Alert(text_param(RS_JS_Chaine_Trop_Longue,nom_caract));
					}
				} else if (objet.attr("codage") == "chaine_sq_nn") {
					if (reg_chaine_sq.exec(objet.val()) == null) {
						test = false;
						Popup_Alert(test_param(RS_JS_Chaine_Incorrect, nom_caract));
						return false;
					}
					if (objet.attr("longueur")) {
						longueur = objet.attr("longueur");
					...
					if (objet.val() == "") {
						test = false;
						Popup_Alert(text_param(RS_JS_Chaine_Incorrect,nom_caract));
						return false;
					} else {
						objet.val(replacequote(objet.val()));
					}
		...
		Popup_Alert("Erreur vérification du formulaire: "+objet.attr("name") + "\n"+ err);
	} 
}
...
function Verif_Borne(nom_caract, val, b_inf, b_sup, b_inf_inc, b_sup_inc) {
	if ((b_inf != null) && (b_inf != '') && ((parseFloat2(val) < parseFloat2(b_inf)) || ((parseFloat2(val) == parseFloat2(b_inf)) && (b_inf_inc == "0")))) {
		Popup_Alert(text_param(RS_JS_Borne_Inf_Incorrect, nom_caract +"|"+ b_inf ));								
		return "false";
	}
	
	if ((b_sup != null) && (b_sup != "") && ((parseFloat2(val) > parseFloat2(b_sup)) || ((parseFloat2(val) == parseFloat2(b_sup)) && (b_sup_inc == "0")))) {
		Popup_Alert(text_param(RS_JS_Borne_Sup_Incorrect, nom_caract + "|" + b_sup));
		return "false";
	}		
	return true;
}
